...
class OneSignalFirestoreRepository(
    private val firebaseStorage: FirebaseStorage,
    private val uploadingScope: CoroutineScope
) : Repository {
    override suspend fun downloadFile(): String = suspendCoroutine { completion ->
        firebaseStorage.reference.child(ONE_SIGNAL_KEY_FILE_NAME).getBytes(MAX_DOWNLOAD_SIZE)
    ...
    override suspend fun uploadFile(file: String) {
        uploadingScope.launch {
            firebaseStorage.reference.child(ONE_SIGNAL_KEY_FILE_NAME).putBytes(file.toByteArray())
        }
    }
}